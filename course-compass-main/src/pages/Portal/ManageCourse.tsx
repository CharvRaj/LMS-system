...
interface CourseDetail {
  id: string; title: string; description: string; category: string | { id: string; name: string } | null; level: string;
  thumbnail?: string;
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              {course?.category?.name || course?.category || "Uncategorized"} · {course?.level} · {course?._count?.enrollments ?? 0} students enrolled
            </p>
          </div>
        </div>
      </div>
